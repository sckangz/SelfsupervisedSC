...
class ConvAE(object):
    def __init__(self, n_input, kernel_size, n_hidden, reg_constant1=1.0, re_constant2=1.0, re_constant3=1.0,re_constant4=1.0,
        ...
        self.x_r = tf.layers.dense(self.x_r, 256, activation=None, name='d4')
        ...
        self.x_r2 = tf.layers.dense(self.x_r2, 256, activation=None, name='d4', reuse=True)
        # self.x_r = self.x_r2
...
def test_face(Img, Label, CAE, num_class,lr2=5e-4):
    ...
    for i in range(0,11-num_class):
        ...
        if pred==True:
            ...
            CAE.save_model()
            pred = True
            visualize(Img, Label, CAE, './temp/usps5-tsne-ae.pdf')

        # visualize(Img, Label, CAE, 'usps5-tsne-ae.pdf')
        max_step = 10#50 + num_class*25# 100+num_class*20
        ...
        for posti in range(1):
            ...
            acc_.append(accnn)
            global figi
            # cc = 0.5*(np.abs(Coef) + np.abs(Coef.T))
            visualize3(L, Label, CAE, './temp2/usps5-tsne-sae%d.pdf' % figi)
            visualize(Img, Label, CAE, './temp/usps5-tsne-sae%d.pdf'%figi)
            figi += 1
        # for sd in [5,6,7]:
        #     for sa in [7,8,9]:
...
if __name__ == '__main__':
    ...
    data = (dataset.data + 1.0) / 2.0
    target = dataset.target.astype(int)-1
    ...
    # logs_path = './logs'
    model_path = './models/model-usps-tempfullfc.ckpt'
    restore_path = './models/model-usps-tempfullfc.ckpt'
    logs_path = './logs'
